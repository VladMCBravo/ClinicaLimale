...
export default function AtendimentoGinecologia({ pacienteId, onEvolucaoSalva }) {
    ...
    return (
        <Paper component="form" onSubmit={handleSubmit} sx={{ p: 2, mb: 2 }}>
            <Typography variant="h6" gutterBottom>Evolução do Dia (Ginecologia)</Typography>
            <Grid container spacing={2}>
                <Grid item xs={12}><TextField name="notas_subjetivas" label="Subjetivo (Queixas ginecológicas, HDA)" multiline rows={4} fullWidth value={formData.notas_subjetivas || ''} onChange={handleChange} size="small" /></Grid>
                <Grid item xs={12}><TextField name="notas_objetivas" label="Objetivo (Exame das mamas, especular, toque)" multiline rows={4} fullWidth value={formData.notas_objetivas || ''} onChange={handleChange} size="small" /></Grid>
