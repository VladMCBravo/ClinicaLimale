...
// --- Funções de Botão Seccionais (NOVO) ---

    // 1. Botão para a Seção "História Pré-Natal"
    const handleNormalidadePreNatal = () => {
        setAnamneseData(prev => ({
            ...prev,
            gpa_g: '1', gpa_p: '1', gpa_a: '0', 
            pre_natal: 'Adequado', tipo_gestacao: 'Única',
            corticoterapia: 'Não', neuroprotecao_mg: 'Não se aplica', 
            condicoes_maternas: 'Não', comorbidades_outras_desc: '',
            vicios: 'Não', vicios_outros_desc: '',
        }));
        // Define os estados dos checkboxes
        setComorbidades({});
        setVicios({});
    };

    // 2. Botão para "Tipagem Sanguínea"
    const handleNormalidadeTipagem = () => {
        setAnamneseData(prev => ({
            ...prev,
    ...
    // 3. Botão para "Sorologias" (Essa função já existia, está correta!)
    const handleNormalidadeSorologias = () => {
        setAnamneseData(prev => ({
            ...prev,
            sorologias: {
                ...prev.sorologias, // Mantém dados de sífilis etc. se já preenchidos
                hiv_status: 'Não reagente',
                sifilis_status: 'Não reagente',
                toxo_status: 'Imune', // 'Imune' é mais comum que 'Não reagente'
                hep_b_status: 'Não reagente',
                outras_inf_status: 'Não reagente',
            }
        }));
        showSnackbarRef.current('Sorologias marcadas com padrão normal.', 'info');
    };

    // 4. Botão para "Nascimento e Parto"
    const handleNormalidadeParto = () => {
         setAnamneseData(prev => ({
            ...prev,
            tipo_parto: 'Normal', bolsa_rota: 'Rota <18h', profilaxia_bolsa: 'Sim', 
            liquido_amniotico: 'Claro',
            apgar_1: 9, apgar_5: 10, apgar_10: 10,
            peso_nascimento: 3500, comprimento: 50, pc_nascimento: 35,
            reanimacao_status: 'Não', reanimacao_obs: '',
        }));
        setReanimacao({});
    };

    // 5. Botão para "Evolução Hospitalar"
    const handleNormalidadeEvolucao = () => {
        setAnamneseData(prev => ({
            ...prev,
    ...
    // 6. Botão para "Triagens"
    const handleNormalidadeTriagens = () => {
        setTriagens({
    ...
    // 7. Botão Limpar (Agora fica aqui, mas opcional)
    const handleLimparHistoricoCompleto = () => {
        setAnamneseData(initialState);
        setComorbidades({});
        setVicios({});
        setReanimacao({});
    ...
    <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, borderColor: 'grey.400' }}>
        ...
        {/* ★★★ Seção 1: Pré-Natal (Correto) ★★★ */}
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
            <Typography variant="body1" sx={{ fontWeight: 'medium' }}>1. História Pré-Natal</Typography>
            <Button size="small" variant="outlined" onClick={handleNormalidadePreNatal} type="button">
        ...
        {/* ★★★ Seção Tipagem (CORRIGIDO com botão) ★★★ */}
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
            <Typography variant="body2" sx={{ fontWeight: 'medium', color: 'text.secondary' }}>Tipagem Sanguínea e Fator Rh</Typography>
            <Button size="small" variant="outlined" onClick={handleNormalidadeTipagem} type="button" sx={{ml: 2}}>
                Normal
            </Button>
        </Box>
        ...
        {/* ★★★ Seção 3: Parto (CORRIGIDO com botão) ★★★ */}
        <Divider sx={{ my: 2 }} />
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="body1" sx={{ fontWeight: 'medium' }}>3. Nascimento e Histórico do Parto</Typography>
            <Button size="small" variant="outlined" onClick={handleNormalidadeParto} type="button">
                Preencher Normalidade
            </Button>
        </Box>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mt: 1.5 }}>
        ...
        {/* ★★★ Seção 4: Evolução (CORRIGIDO com botão) ★★★ */}
        <Divider sx={{ my: 2 }} />
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="body1" sx={{ fontWeight: 'medium' }}>4. Histórico Neonatal e Evolução Hospitalar</Typography>
            <Button size="small" variant="outlined" onClick={handleNormalidadeEvolucao} type="button">
                Preencher Normalidade
            </Button>
        </Box>
        ...
        {/* ★★★ Seção 5: Triagens (CORRIGIDO com botão) ★★★ */}
        <Divider sx={{ my: 2 }} />
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="body1" sx={{ fontWeight: 'medium' }}>5. Triagens e Testes Neonatais</Typography>
            <Button size="small" variant="outlined" onClick={handleNormalidadeTriagens} type="button">
                Preencher Normalidade
            </Button>
        </Box>
...
}); // ★★★ CORREÇÃO 1: Alterado de "};" para "});"

export default HistoricoNeonatologia; // ★★★ CORREÇÃO 2: Alterado de "NeonatalAnamneseForm" para "HistoricoNeonatologia"
