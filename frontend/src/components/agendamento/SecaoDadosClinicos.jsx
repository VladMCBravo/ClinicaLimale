...
export default function SecaoDadosClinicos({
    tipoAgendamento = 'Consulta', setTipoAgendamento,
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                {tipoAgendamento === 'Consulta' && (
                    <Autocomplete
                        ...
                        value={formData?.especialidade || null}
                ...
                <Box sx={{ display: 'flex', gap: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
                    {tipoAgendamento === 'Consulta' ? (
                        <Autocomplete
                            ...
                            getOptionLabel={(m) => m ? `${m.first_name || ''} ${m.last_name || ''}`.trim() : ''}
                            value={formData?.medico || null}
                            isOptionEqualToValue={(o, v) => o?.id === v?.id}
                        ...
                        <Autocomplete
                            ...
                            isOptionEqualToValue={(o, v) => o?.id === v?.id}
                            onChange={(e, value) => setFormData({ ...formData, medico: value })}
                            renderInput={(params) => <TextField {...params} label="Médico Responsável *" size="small" error={!formData?.medico} />}
                        />
                    )}
