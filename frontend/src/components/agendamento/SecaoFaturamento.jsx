...
export default function SecaoFaturamento({
    ...
    // Filtro seguro dos planos de acordo com a empresa/convênio selecionada
    const planosFiltrados = React.useMemo(() => {
        if (!convenioSelecionado || !Array.isArray(planos)) return [];
        return planos.filter(p => p.convenio_nome === convenioSelecionado.nome);
    }, [convenioSelecionado, planos]);
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'center' }}>
                    <FormControl fullWidth size="small">
                        ...
                        <Select
                            ...
                            value={formData?.tipo_atendimento || 'Particular'}
                            label="Tipo"
                    ...
                    <Box sx={{ p: 0.5, bgcolor: formData?.isento_cobranca ? '#e8f5e9' : 'transparent', borderRadius: 1, whiteSpace: 'nowrap' }}>
                        <FormControlLabel
                            control={
                                <Switch
                                    checked={Boolean(formData?.isento_cobranca)}
                            ...
                            label={
                                <Typography variant="body2" fontWeight="bold" color={formData?.isento_cobranca ? 'success.dark' : 'text.primary'}>
                                    Isentar
                                </Typography>
                            }
                            sx={{ m: 0 }}
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
                        <FormControl fullWidth size="small">
                            <Autocomplete
                                ...
                                value={convenioSelecionado || null}
                                isOptionEqualToValue={(option, value) => option?.id === value?.id}
                                onChange={(event, newValue) => {
                                    setConvenioSelecionado(newValue);
                                    setFormData({ ...formData, plano_utilizado: null });
                                }}
                        ...
                        <FormControl fullWidth size="small">
                            <Autocomplete
                                ...
                                getOptionLabel={(option) => option?.nome || ''}
                                value={formData?.plano_utilizado || null}
                                ...
                                noOptionsText={convenioSelecionado ? "Nenhum plano" : "Empresa..."}
                            />
                        </FormControl>
                    </Box>
                )}
                ...
                {infoFinanceira && infoFinanceira.status === 'ok' && (
                    <Box sx={{ p: 1, bgcolor: '#fdf1e2', border: '1px solid #e8b374', borderRadius: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="caption" sx={{ color: '#a35a1d' }}>Total Previsto:</Typography>
                        <Typography variant="body2" sx={{ color: '#a35a1d' }} fontWeight="bold">{infoFinanceira.texto}</Typography>
                    </Box>
                )}

                {infoFinanceira && infoFinanceira.status === 'erro' && (
                    <Alert severity="error" sx={{ py: 0, '& .MuiAlert-message': { py: 0 } }}>
