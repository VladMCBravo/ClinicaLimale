...
// SUBSTITUA AS IMPORTAÇÕES ANTIGAS PELAS NOVAS (V2):
import { 
    gerarRelatorioFetoHTML,  
    montarTextoFinalMultiploHTML 
} from '../logic/obstetricTextBuilderV2';
...
export const useObstetricoForm = (onUpdate = () => {}, initialValues = {}) => {
    ...
    useEffect(() => {
        ...
        // Substitua a geração de relatórios por estas:
        const resF1 = gerarRelatorioFetoHTML(dadosFeto1.current);
        const resF2 = qtdFetos >= 2 ? gerarRelatorioFetoHTML(dadosFeto2.current) : null;
        const resF3 = qtdFetos >= 3 ? gerarRelatorioFetoHTML(dadosFeto3.current) : null;

        const listaFetos = [dadosFeto1.current, dadosFeto2.current, dadosFeto3.current];
        
        // E substitua a montagem final por esta:
        const textoFinal = montarTextoFinalMultiploHTML(resF1, resF2, resF3, qtdFetos, listaFetos);
